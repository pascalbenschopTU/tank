...
class Orchestrator {
     ...
     */
    async updateAgent(isLearning=true) {
        const previousState = this.previous_state; 
        const previousAction = this.model.chooseAction(previousState, this.eps);
        const done = this.agent.move(this.model.getAction(previousAction));
        const currentReward = this.computeReward();
        const currentState = this.agent.getObservationTensor();
        ...
        if (isLearning) {
            ...
            console.log(this.steps, currentReward);
            if (this.steps > 10) {
    ...
    async trainModel() {
        ...
        const hasfitted = await (this.model.train(x, y));
