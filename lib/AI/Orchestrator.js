...
class Orchestrator {
    ...
    updateAgent() {
        ...
        if (this.steps % 150 == 0 && this.model.canTrain) {
            this.trainModel();
    ...
    computeReward(new_position) {
        ...
        if (!agentInWall && this.agent.isDirectionTowardPlayer()) {
            ...
            return Math.ceil(Math.max(100 - (distance / 10), 0)); // reward for going towards player
        } else if (agentInWall){
            return -10; // punish going into a wall
        } else {
            return -10; // punish not moving
        }
    }
