    ...
    async trainModel() {
        ...
        batch.forEach(
            ([state, action, reward, next_state], index) => {
                let currentQ = Array.from(qsa.dataSync().slice(index*this.model.numActions,index*this.model.numActions+this.model.numActions)); // predicted state
                // Update the action with reward (negative is do the action less, positive is do more)
                // Model loss = {(r + y * max(Q'(s',a')) - Q(s,a)}^2  (mse)
                currentQ[action] = reward + this.discount_rate * Util.maxValue(qsad.dataSync().slice(index*this.model.numActions,index*this.model.numActions+this.model.numActions));
