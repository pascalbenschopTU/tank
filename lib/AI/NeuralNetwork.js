...
class Model {
    ...
    copy(network) {
        let newModel =  new Model(this.hiddenLayerSizesOrModel, this.numStates, this.numActions, this.batchSize);
        if (network != null) {
    ...
    async train(xBatch, yBatch) {
        return await (this.network.fit(xBatch, yBatch, {epochs: 1, verbose: this.verbose})).then(_ => {return true})
    }
